...
def trigger_to_dict(trigger: Trigger):
    ...
    effects = list(map(lambda e: effect_to_dict(e), trigger.effects))
    conditions = list(map(lambda c: condition_to_dict(c), trigger.conditions))
...
def effect_to_dict(effect: Effect):
    exact = [
        ...
        "trigger_id", "location_x", "location_y", "location_object_reference", "area_1_x", "area_1_y", "area_2_x",
        "area_2_y", "object_group", "object_type", "instruction_panel_position", "attack_stance", "time_unit",
        ...
        "object_attributes", "variable", "timer", "facet", "play_sound", "player_color", "message", "sound_name",
        "selected_object_ids"
    ]
    to_bool = []
    convert = {}
...
def condition_to_dict(effect: Effect):
    exact = [
        "condition_type", "quantity", "attribute", "unit_object", "next_object", "object_list", "source_player",
        "technology", "timer", "area_1_x", "area_1_y", "area_2_x", "area_2_y", "object_group", "object_type",
        "ai_signal", "inverted", "variable", "comparison", "target_player", "unit_ai_action", "xs_function"
    ]
    to_bool = []
    convert = {}
